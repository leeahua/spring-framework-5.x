...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// which cannot be stored in the shared merged bean definition. vv
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 即使被 BeanFactoryAware 等生命周期接口触发，也积极地缓存单例以解析循环引用。
		//
		...
		try {
			// 对bean进行填充，其中，可能存在依赖于其他的bean属性，则会递归初始依赖bean
		...
		// Register bean as disposable. 将 bean 注册为一次性的。
		try {
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// 让任何 InstantiationAwareBeanPostProcessors 实例化感知 Bean 后处理器 有机会在设置属性之前修改 bean 的状态。
		// 例如，这可以用于支持字段注入的样式 执行bean后处理器
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
		...
		// hasInstAwareBpps 是否具有实例化感知 Bean 后处理器
		boolean hasInstAwareBpps = hasInstantiationAwareBeanPostProcessors();
		// needsDepCheck 是否需要依赖检查
	 ...
	 * 从给定的 BeanWrapper 中提取一组过滤的 PropertyDescriptor，不包括忽略的依赖类型或定义在忽略的依赖接口上的属性
	 * @param bw the BeanWrapper the bean was created with
	 ...
	 * <p>从 {@link createBean} 调用传统定义的 bean，从 {@link initializeBean} 调用现有 bean 实例
